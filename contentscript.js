...
$(document).arrive("article", function(articleData) {
	...
	if (svg_class !== undefined && svg_class == SMALL_ICON_CLASS)
	{
...
function hideLikes(hideVal) {
	...
	article_refs.each( function(index, data)
	{
		...
		// Check if this SVG belongs to a Liked tweet, and if so, hide the whole article
		if (svg_data !== undefined && svg_class !== undefined && svg_class == SMALL_ICON_CLASS.toString() && svg_data.startsWith(LIKED_TWEET_DATA))
		{
...
function hideComments(hideVal) {
	...
	article_refs.each( function(index, data)
	{
		...
		// Check if this SVG belongs to a Reply tweet, and if so, hide the whole article
		if (svg_data !== undefined &&  svg_class !== undefined && svg_class == SMALL_ICON_CLASS.toString() && svg_data.startsWith(REPLY_TWEET_DATA))
		{
			if (hideVal === true) 
			{ 
...
function hideFollowed(hideVal) {
	...
	article_refs.each( function(index, data)
	{
		...
		if (svg_data !== undefined && svg_class !== undefined && svg_class == SMALL_ICON_CLASS.toString() && svg_data.startsWith(FOLLOWED_TWEET_DATA))
		{
			if (hideVal === true) { $(data).closest(ARTICLE_DIV_CLASS).hide(); }
			else 				  { $(data).closest(ARTICLE_DIV_CLASS).show(); }		
		}
...
function hideRetweets(hideVal) {
	...
	article_refs.each( function(index, data)
	{
		...
		// Check if this SVG belongs to a Retweet
		if (svg_data !== undefined && svg_class !== undefined && svg_class == SMALL_ICON_CLASS.toString() && svg_data.startsWith(RETWEET_DATA))
		{
			// If no filter is applied, just hide the Retweet
